...
interface AnalyzeCollarButtonProps {
  modelId: number;
  previewImageUrl?: string;
  hasDescription: boolean;
}

export default function AnalyzeCollarButton({
  modelId,
  previewImageUrl,
  hasDescription,
}: AnalyzeCollarButtonProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch('/api/analyze-collar', {
        method: 'POST',
        headers: {
      ...
      setError(err instanceof Error ? err.message : 'Failed to analyze collar');
    } finally {
    ...
    return (
      <div className="flex flex-col items-center gap-1">
        ...
        <button
          ...
          title="Re-check collar/tag from the preview image"
        >
          {isAnalyzing ? 'Analyzing...' : 'Re-analyze collar'}
        </button>
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <button
        ...
        title="Detect collar and read tag text from the preview image"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
        {isAnalyzing ? 'Analyzing...' : 'Detect collar/tag'}
      </button>
      {error && <p className="text-xs text-red-500">{error}</p>}
      {!previewImageUrl && (
